...
async function judgeRoom(roomId: number) {
  try {
    ...
    // Comparative judging pass: forces the AI to score the two coins relative to
    // each other so we don't get identical "safe middle" scores from independent
    // parallel runs. Returns adjusted scores + winner + punchy reason.
    let winner: "A" | "B" = a.score >= b.score ? "A" : "B";
    let reason = `Higher Survive Score (${a.score} vs ${b.score})`;
    let scoreA = a.score;
    let scoreB = b.score;
    try {
      const prompt = `Two meme coin ideas just got roasted in a LIVE 1v1 battle. Score them RELATIVE TO EACH OTHER on Four.meme launch potential (0-100). The two scores MUST be different — pick a clear winner. Spread should reflect quality gap (close fight = 5-15 pt gap, blowout = 25+).

COIN A (@${room.hostUser}): "${a.tokenIdea}"
...
COIN B (@${room.opponentUser}): "${b.tokenIdea}"
...
Reply ONLY as JSON: {"scoreA":number,"scoreB":number,"winner":"A"|"B","reason":"one savage sentence, max 25 words"}`;
    ...
    // Final tie-break safety net: scores must never be equal, and winner must
    // always have the higher score. If the AI still tied them, nudge by 3 pts.
    if (scoreA === scoreB) {
      if (winner === "A") scoreA = Math.min(100, scoreA + 3);
      else scoreB = Math.min(100, scoreB + 3);
    } else {
      // Ensure winner is consistent with the (possibly adjusted) scores.
      winner = scoreA > scoreB ? "A" : "B";
    }

    a.score = scoreA;
    b.score = scoreB;
    reason = reason || `Higher Survive Score (${scoreA} vs ${scoreB})`;
